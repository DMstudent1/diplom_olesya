<template>
    <v-app-bar color="green" dark elevation="2" class="px-4">
        <div class="app-bar-content">
            ...
            <router-link v-if="authStore.isAuthenticated" to="/orders" class="nav-link text-white mr-6">
                Заказы
            </router-link>
            ...
            >
                <template v-slot:activator="{ props }">
                    <span v-bind="props" class="nav-link text-white mr-6 cursor-pointer">
                        Категории
                    </span>
                </template>
                    ...
                    >
                        ...
                        <div class="pagination-controls pa-2">
                            <v-btn 
                                ...
                                variant="text" 
                                size="small"
                                @click="loadCategories(prevCursor)"
                                :loading="loadingMore"
                                class="mr-2"
                            >
                                ← Назад
                            ...
                            <v-btn 
                                ...
                                variant="text" 
                                size="small"
                                @click="loadCategories(nextCursor)"
                                :loading="loadingMore"
                            >
                                Далее →
                            </v-btn>
            ...
            <!-- Корзина -->
            <v-btn to="/cart" v-if="authStore.isAuthenticated && cartStore.cart" variant="text" class="pa-2" height="auto"
            ...
            <v-menu open-on-hover offset-y :close-on-content-click="false">
                ...
                <v-card v-else class="pa-2" width="200">
                    ...
                    <v-btn v-if="authStore.user && hasPermission(authStore.user.permissions, 'categories-create')" to="/categories" block variant="text" class="mb-2 justify-start" prepend-icon="mdi-folder-outline">
                        Категории
                    </v-btn>
                    <v-btn v-if="authStore.user && hasPermission(authStore.user.permissions, 'products-create')" to="/products" block variant="text" class="mb-2 justify-start" prepend-icon="mdi-package-variant">
                        Товары
                    </v-btn>
                    ...
                    <v-btn @click="handleLogout" block variant="text" class="mb-2 justify-start" prepend-icon="mdi-exit-to-app">
                        Выйти
                    </v-btn>
                </v-card>
            </v-menu>
